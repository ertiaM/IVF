...
OUT_PATH  = BASE / "pre_test/results/predictions_media_gpt-gemini-3.5-flash.json"

API_URL = "https://api3.xhub.chat/v1/chat/completions"
API_KEY = "sk-Co7aVxrhEYKOomDszhPgOvksE1sLEaOLl4MVV5g1AnOaTE1z"
MODEL   = "gemini-3-flash-preview"

VIDEO_ROOT = Path("/aifs4su/zhanqimin/ai_for_healthcare/data/港科大-深圳中山AI项目资料(2026-03-05)/胚胎影像视频")
IMAGE_ROOT = Path("/aifs4su/zhanqimin/ai_for_healthcare/data/港科大-深圳中山AI项目资料(2026-03-05)/超声影像图片")

# ── 配额：API 硬限制 16 张；1 个视频×2 帧；其余全给图片 ── # ← 改
API_MAX_IMAGES          = 16
VIDEO_MAX_VIDEOS        = 1    # ← 改：只取 1 个视频
VIDEO_FRAMES_PER_VIDEO  = 4    # 每个视频抽 2 帧
VIDEO_BUDGET            = VIDEO_MAX_VIDEOS * VIDEO_FRAMES_PER_VIDEO   # = 2
IMAGE_BUDGET            = API_MAX_IMAGES - VIDEO_BUDGET               # = 14

# ── 激进压缩：长边 512px，JPEG quality=55 ──────────────── # ← 改
IMAGE_LONG_SIDE    = 512
...
除文字病历外，部分病例还会附带超声影像图片和胚胎培养影像视频帧，请一并纳入分析。
...
▌ 超声影像图片（若附带，最多14张，均为压缩后的 JPEG）
▌ 胚胎培养影像视频帧（若附带，来自1个视频，最多2帧）
...
def build_video_index(video_root: Path) -> dict[str, Path]:
    """
    文件夹格式：{大号couple_id}_{pt_id}
    JSON 里的 couple_id（如 1065427）与文件夹前缀（如 10000045659）完全不同，
    因此只按 pt_id（下划线后半部分）建立索引。      # ← 改：移除 couple_id 索引
    """
    index: dict[str, Path] = {}
    ...
    for folder in video_root.iterdir():
        ...
        if len(parts) == 2:
            pt_id = parts[1]               # ← 改：只用 pt_id
            index[pt_id] = folder
    print(f"[INFO] 视频索引构建完成，共 {len(index)} 个文件夹（按 pt_id 索引）")
    return index
...
def build_image_index(image_root: Path) -> dict[str, Path]:
    ...
    if not image_root.exists():
        print(f"[WARN] 图片目录不存在: {image_root}")
        return index
    for folder in image_root.iterdir():
        if folder.is_dir():
            index[folder.name] = folder
    print(f"[INFO] 图片索引构建完成，共 {len(index)} 个文件夹")
    return index
...
# ─── 媒体文件路径查找 ─────────────────────────────────────────────────────
def get_video_paths(pt_id: str) -> list[Path]:           # ← 改：只接收 pt_id
    folder = VIDEO_INDEX.get(str(pt_id))
    if folder is None:
        return []
    videos = sorted(folder.glob("*.avi"))
    if videos:
        print(f"    🎬 pt_id={pt_id} → {folder.name}  ({len(videos)} 个视频，只用第1个)")
    return videos[:VIDEO_MAX_VIDEOS]                      # ← 改：截断到 1 个


def get_image_paths(ckno: str) -> list[Path]:
    folder = IMAGE_INDEX.get(str(ckno))
    if folder is None:
        return []
    return sorted(
...
def resolve_media(patient: dict, cycle: dict) -> tuple[list[Path], list[Path]]:
    # ── 收集该 couple 下所有 pt_id ──────────────────────────────────────
    all_pt_ids = set()
    for c in patient.get("cycles", []):
        pid = c.get("pt_id")
        if pid:
            all_pt_ids.add(str(pid))
    # 当前 cycle 的 pt_id 也加进去（冗余保险）
    if cycle.get("pt_id"):
        all_pt_ids.add(str(cycle["pt_id"]))

    print(f"    🔍 尝试 pt_ids: {all_pt_ids}")

    # ── 视频：遍历所有 pt_id，找到第一个有视频的就用 ────────────────────
    video_paths: list[Path] = []
    for pid in sorted(all_pt_ids):
        paths = get_video_paths(pid)
        if paths:
            video_paths = paths
            print(f"    🎬 视频命中 pt_id={pid}")
            break   # 只用1个视频，找到即停

    # ── 图片：按 exam_reports 里的 ckno 查 ──────────────────────────────
    image_paths: list[Path] = []
    for report in patient.get("exam_reports", []):
        if "超声" in str(report.get("classify", "")):
            ckno = report.get("ckno")
            if ckno:
                image_paths.extend(get_image_paths(str(ckno)))

    return image_paths, video_paths


# ─── 图片/帧压缩 ──────────────────────────────────────────────────────────
def compress_image_b64(path: Path,
                        long_side: int = IMAGE_LONG_SIDE,
                        quality: int = IMAGE_JPEG_QUALITY) -> str:
...
def load_video_frames_b64(path: Path,
                           max_frames: int = VIDEO_FRAMES_PER_VIDEO,
                           long_side: int = IMAGE_LONG_SIDE,
                           quality: int = IMAGE_JPEG_QUALITY) -> list[str]:
...
def build_user_content(patient_json: str,
                        image_paths: list[Path],
                        video_paths: list[Path]) -> list[dict] | str:
    text = f"请根据以下 IVF 患者数据预测本次胚胎移植的临床妊娠结局：\n\n{patient_json}"
    ...
    # 超声图片（最多 IMAGE_BUDGET = 14 张）
    for p in image_paths[:IMAGE_BUDGET]:
        if used >= API_MAX_IMAGES:
            break
        try:
            b64 = compress_image_b64(p)
            content.append({"type": "image_url",
                             "image_url": {"url": f"data:image/jpeg;base64,{b64}"}})
            used += 1
        except Exception as e:
            print(f"    [WARN] 图片加载失败 {p.name}: {e}")

    # 视频帧（最多 VIDEO_BUDGET = 2 帧，来自 1 个视频）
    for vp in video_paths:                                # video_paths 已截断为 1 个
        for b64 in load_video_frames_b64(vp):
            if used >= API_MAX_IMAGES:
                break
            content.append({"type": "image_url",
                             "image_url": {"url": f"data:image/jpeg;base64,{b64}"}})
...
def call_llm(patient_json: str,
    ...
    user_content = build_user_content(patient_json, image_paths, video_paths)
...
def main():
    ...
    for idx, s in enumerate(samples, 1):
        ...
        image_paths, video_paths = resolve_media(s["patient"], s["cycle"])  # ← 改：去掉 couple_id
        n_img = min(len(image_paths), IMAGE_BUDGET)
        n_vid = len(video_paths)
        if image_paths or video_paths:
            print(f"\n    📎 图片: {len(image_paths)} 张(发送 {n_img})  "
                  f"视频: {n_vid} 个(发送帧 ≤{n_vid * VIDEO_FRAMES_PER_VIDEO})", end="  ")

        inp      = build_input(s["patient"], s["cycle"])
        inp_json = json.dumps(inp, ensure_ascii=False)
        llm_out  = call_llm(inp_json, image_paths=image_paths, video_paths=video_paths)
